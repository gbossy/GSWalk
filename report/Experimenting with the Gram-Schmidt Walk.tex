...
\usepackage[linesnumbered,boxruled,vlined]{algorithm2e}%ruled in 1st param
...
\begin{theorem}[\cite{construct}]\label{equivalence}
        ...
        \begin{enumerate}
            \item  $\textbf{B}\textbf{z}_0 %= \sum_{i=1}^n \textbf{z}_0(i)\textbf{v}_i
            \in K$\\
            \item  For $\textbf{z} \sim D$, $\textbf{B}(\textbf{z}-\textbf{z}_0)%= \sum_{i=1}^n (\textbf{z}(i)-\textbf{z}_0(i))\textbf{v}_i
¨	 $ is $\sigma$-subgaussian, for some $\sigma >0$
            \item For any $i\in[n]$, if $\textbf{z}_0(i) \in \{-1, 1\}$, $\mathbb{P}[\textbf{z}(i)=\textbf{z}_0(i)]=1$.
            \item $\mathbb{P}_{\textbf{z} \sim D}[%\sum_{i=1}^n% (\textbf{z}(i)%- \textbf{z}_0(i))\textbf{v}_i 
...
\begin{theorem}[\cite{harshaw2019balancing}]
    For $\textbf{z}$ sampled via the Gram-Schmidt walk with input $\textbf{v}_1, \dots, \textbf{v}_n \in \mathbb{R}^d$ each with $\ell_2$ norm at most 1, we have that $\textbf{Bz}$ is subgaussian with parameter $\sigma^2=1$.%:$$\mathbb{E}[exp(\langle\textbf{Bz},\textbf{v}\rangle)]\leq exp(\|\textbf{v}\|^2_2/2)\quad\forall\textbf{v}\in\mathbb{R}^{n+d}$$
\end{theorem}
...
%&=\frac{1}{\textbf{d}_k(k)}\textbf{c}_k^T\\
...
, an initial coloring $\textbf{z}_0\in[-1,1]^n$,   \textcolor{red}{$n$ linearly independent basis vectors $\textbf{b}_1,\dots,\textbf{b}_n\in\mathbb{R}^n$}}
   ...
   $A_1=\{i\in[n]:\textcolor{red}{|\textbf{z}_0'(i)|}<1\}$, $n(1) = \max \{i \in A_1\}$ and $t=1$.\\
